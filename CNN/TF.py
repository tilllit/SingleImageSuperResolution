...
model_path = 'model_h5.weights.h5'
...
def main(train=False, resume=False):
    ...
    #test_path = 'graffiti.png'
    test_path = '0003.png'
    original_img, downscaled = load_image(test_path, scaling_factor)
    res = upscale_image(downscaled, model)

    fig, (a0, a1) = plt.subplots(1, 2, figsize=(17, 7))
    plt.tight_layout()

    a0.imshow(downscaled)
    a0.set_title("Low Res")

    a1.imshow(res)
    a1.set_title("Upscaled")
...
if __name__ == "__main__":
    #main(train=True, resume=False)  # Training
    main()                           # Testing
