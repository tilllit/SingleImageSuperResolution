...
- The GAN obviously produces outputs, that are most appealing to the pure eye. Since it generates missing content, it differs more from the original, than one can see. This is filtered and shown by the conventional metrics, comparing the output to the original.
  The sharpness metric, using the variance of the laplace-arlgorithm uses a non-comparing principal. It shows in every picture, that the SRGAN produces the sharpest pictures of the three methodes. <br />
